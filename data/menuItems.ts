import { MenuItem } from "@/types";

export const menuItems: MenuItem[] = [

  // =========================
  // STARTERS
  // =========================

  {
    id: "1",
    name: "Samosa",
    description: "2 pcs",
    price: 60,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "2",
    name: "Wada Pav",
    description: "2 pcs",
    price: 50,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "3",
    name: "Moong Dal Kachori",
    description: "2 pcs",
    price: 70,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "4",
    name: "Bread Roll",
    description: "4 pcs",
    price: 70,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "5",
    name: "Corn Cheese Ball",
    description: "4 pcs",
    price: 90,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "6",
    name: "Plain Maggi",
    description: "Plate",
    price: 60,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "7",
    name: "Masala Maggi",
    description: "Plate",
    price: 80,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "8",
    name: "Veg Loaded Nachos",
    description: "Plate",
    price: 90,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "9",
    name: "French Fries",
    description: "Plate",
    price: 90,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "10",
    name: "Veg Frankie",
    description: "",
    price: 80,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "11",
    name: "Paneer Roll",
    description: "",
    price: 90,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "12",
    name: "Egg Roll",
    description: "",
    price: 100,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "13",
    name: "Bun Maska",
    description: "",
    price: 50,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },
  {
    id: "14",
    name: "Veg Hakka Noodles",
    description: "Plate",
    price: 200,
    category: "Snacks",
    imageURL: "",
    isPure: true,
  },


  // =========================
  // SANDWICHES BURGERS & WRAPS
  // =========================

  {
    id: "15",
    name: "Veg Grilled Sandwich",
    description: "",
    price: 80,
    category: "Sandwiches, Burgers & Wraps",
    imageURL: "",
    isPure: true,
  },
  {
    id: "16",
    name: "Paneer Tikka Sandwich",
    description: "",
    price: 90,
    category: "Sandwiches, Burgers & Wraps",
    imageURL: "",
    isPure: true,
  },
  {
    id: "17",
    name: "Bombay Grilled Sandwich",
    description: "",
    price: 100,
    category: "Sandwiches, Burgers & Wraps",
    imageURL: "",
    isPure: true,
  },
  {
    id: "18",
    name: "Veg Burger",
    description: "",
    price: 90,
    category: "Sandwiches, Burgers & Wraps",
    imageURL: "",
    isPure: true,
  },
  {
    id: "19",
    name: "Paneer Burger",
    description: "",
    price: 100,
    category: "Sandwiches, Burgers & Wraps",
    imageURL: "",
    isPure: true,
  },


  // =========================
  // PARATHA GALI
  // =========================

  {
    id: "20",
    name: "Gobhi Paratha",
    description: "2 pcs",
    price: 120,
    category: "Paratha Gali",
    imageURL: "",
    isPure: true,
  },
  {
    id: "21",
    name: "Aloo Paratha",
    description: "2 pcs",
    price: 120,
    category: "Paratha Gali",
    imageURL: "",
    isPure: true,
  },
  {
    id: "22",
    name: "Mix Veg Paratha",
    description: "2 pcs",
    price: 140,
    category: "Paratha Gali",
    imageURL: "",
    isPure: true,
  },
  {
    id: "23",
    name: "Paneer Paratha",
    description: "2 pcs",
    price: 160,
    category: "Paratha Gali",
    imageURL: "",
    isPure: true,
  },
  {
    id: "24",
    name: "Sattu Paratha",
    description: "2 pcs",
    price: 160,
    category: "Paratha Gali",
    imageURL: "",
    isPure: true,
  },


  // =========================
  // MAIN COURSE
  // =========================

  {
    id: "25",
    name: "Rajma",
    description: "Plate",
    price: 190,
    category: "Main Course",
    imageURL: "",
    isPure: true,
  },
  {
    id: "26",
    name: "Chole",
    description: "Plate",
    price: 190,
    category: "Main Course",
    imageURL: "",
    isPure: true,
  },
  {
    id: "27",
    name: "Jeera Aloo",
    description: "Plate",
    price: 160,
    category: "Main Course",
    imageURL: "",
    isPure: true,
  },
  {
    id: "28",
    name: "Gobhi Aloo",
    description: "Plate",
    price: 180,
    category: "Main Course",
    imageURL: "",
    isPure: true,
  },
  {
    id: "29",
    name: "Bhindi Masala",
    description: "Plate",
    price: 180,
    category: "Main Course",
    imageURL: "",
    isPure: true,
  },
  {
    id: "30",
    name: "Mix Veg",
    description: "Plate",
    price: 200,
    category: "Main Course",
    imageURL: "",
    isPure: true,
  },


  // =========================
  // CHAAR CHULHA SANGAM
  // =========================

  {
    id: "31",
    name: "Rajma Chawal",
    description: "Plate",
    price: 110,
    category: "Chaar Chulha Sangam",
    imageURL: "",
    isPure: true,
  },
  {
    id: "32",
    name: "Chole Chawal",
    description: "Plate",
    price: 110,
    category: "Chaar Chulha Sangam",
    imageURL: "",
    isPure: true,
  },
  {
    id: "33",
    name: "Kadhi Chawal",
    description: "Plate",
    price: 110,
    category: "Chaar Chulha Sangam",
    imageURL: "",
    isPure: true,
  },
  {
    id: "34",
    name: "Puri Sabji",
    description: "4 pcs",
    price: 100,
    category: "Chaar Chulha Sangam",
    imageURL: "",
    isPure: true,
  },
  {
    id: "35",
    name: "Chole Bhature",
    description: "4 pcs",
    price: 120,
    category: "Chaar Chulha Sangam",
    imageURL: "",
    isPure: true,
  },
  {
    id: "36",
    name: "Pav Bhaji",
    description: "4 pcs",
    price: 100,
    category: "Chaar Chulha Sangam",
    imageURL: "",
    isPure: true,
  },


  // =========================
  // THALI DARBAR
  // =========================

  {
    id: "37",
    name: "Annapurna Thali",
    description: "4 Phulka + Dal + Rice + Sabji + Salad",
    price: 165,
    category: "Thalis",
    imageURL: "",
    isPure: true,
  },
  {
    id: "38",
    name: "Royal Annapurna Thali",
    description: "4 Phulka + Dal + Rice + Seasonal Sabji + Paneer Ki Sabji + Salad",
    price: 199,
    category: "Thalis",
    imageURL: "",
    isPure: true,
  },
  {
    id: "39",
    name: "Chicken Thali",
    description: "4 Phulka + Chicken Curry + Rice + Salad",
    price: 210,
    category: "Thalis",
    imageURL: "",
  },
  {
    id: "40",
    name: "Royal Thali",
    description: "4 Phulka + Dal Tadka + Kaju Pulao + Shahi Paneer + Seasonal Sabji + Sweet + Raita + Salad",
    price: 450,
    category: "Thalis",
    imageURL: "",
    isPure: true,
  },

];

export const CATEGORIES: MenuItem["category"][] = ["Snacks", "Sandwiches, Burgers & Wraps", "Paratha Gali", "Chaar Chulha Sangam", "Main Course", "Thalis", "Tiffins"];
